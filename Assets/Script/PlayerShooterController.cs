...
public class PlayerShooterController : MonoBehaviour
{
    ...
    private void Shoot()
    {
        ...
        if (_enemies.Count > 0 && Time.time - _timeFromLastShhot > _fireRate && _nearestEnemy != null)
        {
            _timeFromLastShhot = Time.time;
            Bullet clone = Instantiate(bulletPrefab, transform.position, transform.rotation);
